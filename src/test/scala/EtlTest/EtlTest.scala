...
class EtlTest extends FunSuite with Matchers {
  ...
  test (testName = "a single letter") {
    Etl.Etl.transform(Map(1 -> Seq("A"))) should be (Map("a" -> 1))
  }

  test("single score with multiple letters") {
    //pending
    Etl.Etl.transform(Map(1 -> Seq("A", "E", "I", "O", "U"))) should be(Map("a" -> 1,
      "e" -> 1, "i" -> 1, "o" -> 1, "u" -> 1))
  }

  test("multiple scores with multiple letters") {
    //pending
    Etl.Etl.transform(Map(1 -> Seq("A", "E"), 2 -> Seq("D", "G"))) should be(Map("a" -> 1,
      "d" -> 2, "e" -> 1, "g" -> 2))
  }

  test("multiple scores with differing numbers of letters") {
    //pending
    Etl.Etl.transform(Map(1 -> Seq("A", "E", "I", "O", "U", "L", "N", "R", "S", "T"),
